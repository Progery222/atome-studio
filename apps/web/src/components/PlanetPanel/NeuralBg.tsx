  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const sp of spheres) {
        ...
        // Bounce off edges
        if (sp.x < 0.05) { sp.x = 0.05; sp.vx = Math.abs(sp.vx) }
        if (sp.x > 0.95) { sp.x = 0.95; sp.vx = -Math.abs(sp.vx) }
        if (sp.y < 0.05) { sp.y = 0.05; sp.vy = Math.abs(sp.vy) }
        if (sp.y > 0.95) { sp.y = 0.95; sp.vy = -Math.abs(sp.vy) }
      }
      ...
      for (let i = 0; i < abs.length; i++) {
        for (let j = i + 1; j < abs.length; j++) {
          ...
          if (strength > 0.7) {
            ...
            ctx.fillStyle = fg
            ctx.beginPath(); ctx.arc(cpx, cpy, 30, 0, Math.PI * 2); ctx.fill()
          }
          ...
          for (let s = 0; s <= steps; s++) {
            ...
            // Core
            if (packet > 0.3) {
              ctx.beginPath(); ctx.arc(px, py, 1.5 + packet * 2, 0, Math.PI * 2)
      ...
      for (const n of abs) {
        ...
        ctx.fillStyle = g1
        ctx.beginPath(); ctx.arc(n.x, n.y, breatheR * 3, 0, Math.PI * 2); ctx.fill()
        ...
        ctx.fillStyle = g2
        ctx.beginPath(); ctx.arc(n.x, n.y, breatheR, 0, Math.PI * 2); ctx.fill()
      }
