...
export function buildGalaxy(w: number, h: number, dpr: number): HTMLCanvasElement {
  ...
  const clouds: [number, number, number, string][] = [
    ...
    [CX + w * 0.22, CY + h * 0.1, w * 0.2, "rgba(40, 10, 140, 0.025)"],
    [CX - w * 0.05, CY + h * 0.2, w * 0.15, "rgba(0, 40, 120, 0.02)"],
  ];
  ...
  for (let i = 0; i < 1400; i++) {
    ...
    const alpha = (rng() * 0.4 + 0.05) * centerDim;
    let color: string;
