import { OrbitConfig, Service } from "@atome/shared";
...
function drawNucleusGlow(
  ...
  const glowAlpha = 0.18 + 0.1 * Math.sin(time * 2.0);
  ...
  const innerAlpha = 0.3 + 0.12 * Math.sin(time * 2.5 + 0.5);
...
function drawOrbitHalf(
  ...
  const startAngle = half === "back" ? Math.PI - ARC_OVERLAP : -ARC_OVERLAP;
  const endAngle = half === "back" ? Math.PI * 2 + ARC_OVERLAP : Math.PI + ARC_OVERLAP;
  ...
  for (let p = 0; p < PULSE_COUNT; p++) {
    ...
    pg.addColorStop(
      0,
      `rgba(${Math.min(255, r + 100)}, ${Math.min(255, g + 100)}, ${Math.min(255, b + 100)}, 0.85)`
    );
    pg.addColorStop(
      0.12,
      `rgba(${Math.min(255, r + 50)}, ${Math.min(255, g + 50)}, ${Math.min(255, b + 50)}, 0.4)`
    );
    ...
    for (let s = 1; s <= steps; s++) {
      ...
      const tAlpha = 0.2 * (1 - frac) * (1 - frac); // quadratic fade
...
function drawSphere(
  ...
  halo.addColorStop(0, `rgba(${gr}, ${gg}, ${gb}, ${0.4 * depthScale})`);
  ...
  bodyGrad.addColorStop(
    0,
    `rgba(${Math.min(255, gr + 100)}, ${Math.min(255, gg + 100)}, ${Math.min(255, gb + 100)}, ${alpha})`
  );
  bodyGrad.addColorStop(0.35, `rgba(${gr}, ${gg}, ${gb}, ${alpha * 0.9})`);
  bodyGrad.addColorStop(
    0.75,
    `rgba(${(gr * 0.4) | 0}, ${(gg * 0.4) | 0}, ${(gb * 0.4) | 0}, ${alpha * 0.85})`
  );
  bodyGrad.addColorStop(
    1,
    `rgba(${(gr * 0.15) | 0}, ${(gg * 0.15) | 0}, ${(gb * 0.15) | 0}, ${alpha * 0.6})`
  );
  ctx.beginPath();
  ...
  rimGrad.addColorStop(
    1,
    `rgba(${Math.min(255, gr + 60)}, ${Math.min(255, gg + 60)}, ${Math.min(255, gb + 60)}, ${0.15 * depthScale})`
  );
  ctx.beginPath();
  ctx.arc(x, y, r, 0, Math.PI * 2);
...
export function render(
  ...
  ctx.drawImage(state.nucCanvas!, -nucDrawSize / 2, -nucDrawSize / 2, nucDrawSize, nucDrawSize);
  ctx.restore();
