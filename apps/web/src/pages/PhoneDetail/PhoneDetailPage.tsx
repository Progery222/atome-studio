...
export function PhoneDetailPage() {
  ...
  const phoneQueue = queue.filter(t => t.phone_id === id)

  if (phonesLoading) return <div className={styles.page}><div className={styles.empty}>загрузка...</div></div>
  if (!phone) return (
    <div className={styles.page}>
      <Link to="/phones" className={styles.back}>← Телефоны</Link>
      <div className={styles.empty}>— телефон {id} не найден</div>
    </div>
  )
  ...
  return (
    <div className={styles.page}>
      ...
      {/* ADB warning */}
      {!phone.adb_connected && (
        <div className={styles.adbWarn}>
          ⚠ ADB не подключён — проверьте кабель или перезагрузите устройство
        </div>
      )}
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.card}>
          ...
          <div className={styles.healthBig} style={{ color: hc }}>{phone.health_score}%</div>
          <div className={styles.healthBar}>
            <div className={styles.healthFill} style={{ width: `${phone.health_score}%`, background: hc }} />
          </div>
        </div>
        ...
        {/* Actions today */}
        <div className={styles.card}>
          <div className={styles.cardLabel}>Действия сегодня</div>
          <div className={styles.cardBig}>{phone.actions_today}</div>
        </div>

        {/* Warmup day */}
        <div className={styles.card}>
          <div className={styles.cardLabel}>Warmup день</div>
      ...
      <div className={styles.detailCard}>
        ...
        {([
          ...
          ['Группа',       phone.group || '—'],
          ['ADB',          phone.adb_connected ? '✓ подключён' : '✕ отключён'],
          ['Последняя акт.', phone.last_active || '—'],
      ...
      {phone.accounts && phone.accounts.length > 0 && (
        <div className={styles.detailCard}>
          <div className={styles.cardTitle}>Аккаунты ({phone.accounts.length})</div>
          {phone.accounts.map(acc => (
            ...
            >
              ...
              <span style={{ color: STATUS_COLOR[acc.status] ?? '#6b7280', fontSize: 11, fontWeight: 600 }}>
                {acc.status}
              </span>
            </div>
          ))}
      ...
      <div className={styles.detailCard}>
        <div className={styles.cardTitle}>Очередь ({phoneQueue.length})</div>
        {phoneQueue.length === 0 ? (
          <div className={styles.emptySmall}>нет задач</div>
        ) : (
          phoneQueue.slice(0, 10).map(task => (
            <div key={task.task_id} className={styles.row}>
              ...
              <span className={styles.rowValue} style={{ color: STATUS_COLOR[task.status === 'in_progress' ? 'active' : task.status === 'failed' ? 'error' : 'paused'] }}>
                {task.status}
              </span>
            </div>
          ))
